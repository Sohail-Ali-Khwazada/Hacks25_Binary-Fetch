...
export const handleMonthlyData = async (req, res) => {
  ...
  //     },
  //     body: JSON.stringify({ brandsWork, monthlyDescription: monthlyGoal, occasion: festival, dateAndTime: date }),
  //   });
...
export const postHandler = async (req, res) => {
  ...
  **Text Overlay:** "Trending Now | ${occasion} | ${brandsWork}"
  `;
  const imagePath = path.join(process.cwd(), 'uploads', 'image_1738203991695_77.jpg');

  const post = new Post({
    image: imagePath,
    caption: monthlyDescription,
    postTime: postTime,
  });
  ...
  try{
    ...
    agenda.schedule(postTime, "Post", { postId: newPost._id });
...
//     `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyCHc5edZCA1RCipaoqI2NJpTJG82rH2r0k`,
//     {
//       method: "POST",
//       headers: {
//         "Content-Type": "application/json",
